...
$(() => { ///////// jQB /////////////////
  ...
  ********************************/
  ...
  // $()는 querySelectorAll과 같이 이름이 같은 모든 선택자를 자동으로 가져온다.
  ...
  *************************************/
  const miniAct = (ele, seq, call) => {
    ...
    msg.fadeOut(200);
    // fadeOut(시간, 이징, 함수)
    // -> opacity가 0이되며 애니메이션 후에 display;none됨
    // 반대는 이부분 수정보완하기
    ...
    // left 값을 보정하여 미니언즈가 각 li방의 중앙에 오게함.
    // left값 + li가로 크기 절반 - 미니언즈 가로크기 절반.
  ...
  **********************************/
  ...
  btns.first().click((e) => { // e 이벤트 전달변수
        ...
        // 콜백함수 : 이동후 실햄함수 //
        let callFn = () => { // 애니후 실행함수(콜백 함수).

        // 1. 메시지 변경
        msg.text('와~! 아늑하다! 옆방으로 가보자!').fadeIn(200);

        // 2. 다음 버튼 보이기
        $(e.currentTarget).next().delay(500) // 0.5초 지연
        .slideDown(400);
    ...
    .next().click((e) => {
      ...
      let callFn = () => { // 애니후 실행함수(콜백 함수).
        ...
        // 1. 좀비 등장
        // find(선택자) -> 자손 요소 찾기.
        // 비교) children(선택자) -> 직계 자식 찾기!.
        bd.eq(9).find('.mz').delay(1000).fadeIn(400, () => {
          // 2. 메시지 변경하기
          // html(코드) -> html 코드 넣기(JS의 innerHTML)
          msg.html('악!;;; 좀비!<br>어서 피하자').fadeIn(200).css({left:"-90%"}) // 박스 위치 변경
    ...
    .next().click((e) => {
      ...
      let callFn = () => { // 애니후 실행함수(콜백 함수).
        ...
        // transform은 css로 tansition으로 구현!
        $('.inj').css({
      ...
      // miniAct(버튼자신,이동할li순번, 콜백함수)
    }) // 3-6. "치료 주사 방으로!" 버튼 클릭시 끝 /////

    // 3-7. "3번 방으로 가야지" 버튼 클릭시 시작 /////
    // next() 선택요소의 다음 형제요소
    // 앞의 버튼 다음버튼임!
    .next().click((e) => {
      ...
      // miniAct(버튼자신,이동할li순번, 콜백함수)
    }) // 3-7. "3번 방으로 가야지" 버튼 클릭시 끝 /////

    // 3-8. "1번 방으로 가야지" 버튼 클릭시 시작 /////
    // next() 선택요소의 다음 형제요소
    // 앞의 버튼 다음버튼임!
    .next().click((e) => {
